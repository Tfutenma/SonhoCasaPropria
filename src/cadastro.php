...
<?php
    ...
    $mensagem='Esta mensagem foi enviada através do formulário de Cadastro<br><br>';
    $mensagem.= ''.$assunto. '<br>';
...
$mail = new PHPMailer\PHPMailer\PHPMailer();
    ...
    $mail->addAddress('cadastro@sonhocasapropria.com.br');
    ...
    $mail->Subject = 'Cadastro: '.$nome .' ' .$assunto; //ASSUNTO
    $mail->Body    = $mensagem;  //CORPO DA MENSAGEM
    $mail->AltBody = $mensagem;  //CORPO DA MENSAGEM EM FORMA ALT
 
    // $mail->send();
    if(!$mail->Send()) {
        echo "<script>alert('Erro ao efetuar o cadastro!');window.location = $site;</script>";
     }else{
        echo "<script>alert('Cadastro efetuado com sucesso!');window.location = $site;</script>";
     }
     die
?>